...
int main(){
	...
	for(size_t bin_num=5,bin_count=QBins.size();bin_num<bin_count;bin_num++){
		auto kin_v=Hist2d(MC,He3eta_msg,{"Histograms","He3Forward_Vertices"},string("Kinematic-vertex-Bin-")+to_string(bin_num)).Scale(5,5);
		PlotHist2d<double>(sp2).Distr(kin_v) << "set xlabel 'E_k, GeV'" << "set ylabel 'theta, deg'"<< "set xrange [0.2:0.4]";
		...
		PlotHist2d<double>(sp2).Distr(kin_mc)<<"set xlabel 'E_k, GeV'"<<"set ylabel 'theta, deg'"<< "set xrange [0.2:0.4]";
		...
		PlotHist2d<double>(sp2).Distr(data_hist)<<"set xlabel 'E_k, GeV'"<<"set ylabel 'theta, deg'"<< "set xrange [0.2:0.4]";
	}
	return 0;
}