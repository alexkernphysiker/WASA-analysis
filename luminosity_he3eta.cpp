...
int main(int,char**){
	...
	for(size_t bin_num=0;bin_num<mc_norm_fg.count();bin_num++)if(mc_accepted_fg[bin_num].Y().val()>1){
		...
		hist measured(DATA,"He3",histpath_forward,string("MissingMass-Bin-")+to_string(bin_num));
		{
			...
			double norm=0;for(auto&p:background1)norm+=p.Y().val();
			background1/=value(norm);
		}{
			double norm=0;for(auto&p:background2)norm+=p.Y().val();
			background2/=value(norm);
		}
