...
int main()
{
    ...
    Plot("he3eta-alpha").Line(SortedPoints<>(alpha.func(),chain))
        << "set xlabel 'p_{eta,CM}, GeV/c'"
        << "set ylabel 'alpha, n.d.'"<<"set yrange [0:1.5]"
        << "set title 'Alpha parameter'";
    Plot("he3eta-beta").Line(SortedPoints<>(beta.func(),chain))
        << "set xlabel 'p_{eta,CM}, GeV/c'"
        << "set ylabel 'beta, n.d.'"<<"set yrange [-0.5:0]"
        << "set title 'Beta parameter'";
    Plot("he3eta-gamma").Line(SortedPoints<>(gamma.func(),chain))
        << "set xlabel 'p_{eta,CM}, GeV/c'"
